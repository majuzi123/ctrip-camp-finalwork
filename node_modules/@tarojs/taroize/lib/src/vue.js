...
function parseVue(dirPath, wxml, jsCode = '') {
    let ast = (0, utils_1.parseCode)(jsCode);
    ...
    const vistor = {
        ...
        CallExpression(path) {
            ...
            (0, script_1.replaceIdentifier)(callee);
            (0, script_1.replaceMemberExpression)(callee);
    ...
    (0, babel_traverse_1.default)(ast, vistor);
    if (!foundWXInstance) {
        ast = (0, utils_1.parseCode)(jsCode + ';Component({})');
        (0, babel_traverse_1.default)(ast, vistor);
    }
...
function parseWXML(dirPath, wxml, imports) {
    ...
    const nodes = (0, himalaya_wxml_1.parse)(wxml.trim()).map(node => parseNode(node, dirPath, imports));
...
function parseAttribute(attr, forItem, forIndex) {
    ...
    const { type, content } = (0, wxml_1.parseContent)(value !== null && value !== void 0 ? value : '', true);
    if (type === 'expression') {
    ...
    if (key.startsWith(':')) {
        key = ':' + (0, lodash_1.kebabCase)(key.slice(1));
    }
    else {
        key = (0, lodash_1.kebabCase)(key);
    }
    return {
        key,
        value
...
function generateVueFile(children) {
    ...
    return (0, himalaya_wxml_1.stringify)([template]);
}
...
function parseTemplate(element, imports) {
    ...
    if (name) {
        ...
        const { type } = (0, wxml_1.parseContent)(value !== null && value !== void 0 ? value : '');
        if (type === 'expression') {
            console.warn('template 的属性 name 只能是一个字符串，考虑更改以下源码逻辑：\n', (0, himalaya_wxml_1.stringify)(element));
        }
        const componentName = (0, template_1.buildTemplateName)(name.key);
        const component = parseWXML('', (0, himalaya_wxml_1.stringify)(children), imports);
        imports.push({
            name: componentName,
        ...
        if (!value) {
            console.warn('template 的 `is` 属性不能为空', (0, himalaya_wxml_1.stringify)(element));
        }
        const { type } = (0, wxml_1.parseContent)(value !== null && value !== void 0 ? value : '');
        if (type === 'expression') {
            console.warn('template 的属性 is 只能是一个字符串，考虑更改以下源码逻辑：\n', (0, himalaya_wxml_1.stringify)(element));
        }
        element.tagName = (0, template_1.buildTemplateName)(value, false);
        element.attributes = [];
        if (data) {
...
function parseModule(element, dirPath, imports) {
    ...
    const { type } = (0, wxml_1.parseContent)(srcValue);
    if (type === 'expression') {
        console.warn(tagName + '的属性 src 只能是一个字符串，考虑更改以下源码逻辑：\n', (0, himalaya_wxml_1.stringify)(element));
    }
    if (srcValue.startsWith('/')) {
        const vpath = (0, path_1.resolve)(utils_1.setting.rootPath, srcValue.substr(1));
        if (!fs.existsSync(vpath)) {
            throw new Error(`import/include 的 src 请填入相对路径再进行转换：src="${srcValue}"`);
        }
        let relativePath = (0, path_1.relative)(dirPath, vpath);
    ...
    if (tagName === 'import') {
        const wxml = (0, template_1.getWXMLsource)(dirPath, srcValue, tagName);
        const mods = (_b = parseWXML((0, path_1.resolve)(dirPath, srcValue), wxml, imports || [])) === null || _b === void 0 ? void 0 : _b.imports;
        imports.push(...(mods || []));
    }
    else {
...
function parseWXS(element, imports) {
    ...
    if (!src) {
        ...
        const ast = (0, utils_1.parseCode)(script);
