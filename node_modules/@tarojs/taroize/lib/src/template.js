...
exports.parseModule = exports.getWXMLsource = exports.parseTemplate = exports.buildTemplateName = void 0;
...
function buildTemplateName(name, pascal = true) {
    if (/wx/i.test(name)) {
        return buildTemplateName('taro-' + name.slice(2, name.length));
    }
    const words = pascal ? (0, utils_1.pascalName)(name + '-tmpl') : name + '-tmpl';
    // return words
    const str = [];
...
function parseTemplate(path, dirPath) {
    ...
    if (name) {
        ...
        path.traverse((0, wxml_1.createWxmlVistor)(loopIds, refIds, dirPath, [], imports));
        ...
        if (refIds.size === 0) {
            // 无状态组件
            render = (0, utils_1.buildRender)(block, [], []);
        }
        else if (refIds.size === 1) {
            // 只有一个数据源
            render = (0, utils_1.buildRender)(block, [], Array.from(refIds), []);
        }
        else {
            // 使用 ...spread
            render = (0, utils_1.buildRender)(block, [], Array.from(refIds), []);
        }
        const classDecl = t.classDeclaration(t.identifier(className), t.memberExpression(t.identifier('React'), t.identifier('Component')), t.classBody([render]), []);
        path.remove();
        return {
            name: className,
            ast: classDecl
...
function getWXMLsource(dirPath, src, type) {
    try {
        let filePath = (0, path_1.resolve)(dirPath, src);
        if (!(0, path_1.extname)(filePath)) {
...
exports.getWXMLsource = getWXMLsource;
function parseModule(jsx, dirPath, type) {
    ...
    if (type === 'import') {
        ...
        const { imports } = (0, wxml_1.parseWXML)((0, path_1.resolve)(dirPath, srcValue), wxml, true);
        try {
            jsx.remove();
        }
        catch (error) {
        ...
        const { wxml } = (0, wxml_1.parseWXML)((0, path_1.resolve)(dirPath, srcValue), wxmlStr, true);
        try {
            if (wxml) {
                block.children = [wxml];
