...
function parse(option) {
    ...
    if (option.framework === 'vue') {
        const result = (0, vue_1.parseVue)(option.path, option.wxml || '', option.script);
        return Object.assign(Object.assign({}, result), { errors: global_1.errors });
    }
    const { wxml, wxses, imports, refIds } = (0, wxml_1.parseWXML)(option.path, option.wxml);
    utils_1.setting.sourceCode = option.script;
    const ast = (0, script_1.parseScript)(option.script, wxml, wxses, refIds, option.isApp);
    return {
        ast,
        imports,
