...
const buildDecorator = (id, isApp = false) => {
    const args = [id];
...
function parseScript(script, returned, wxses = [], refId, isApp = false) {
    script = script || 'Page({})';
    ...
    let ast = (0, utils_1.parseCode)(script);
    let classDecl;
    ...
    if (!foundWXInstance) {
        ast = (0, utils_1.parseCode)(script + ';Component({})');
        (0, babel_traverse_1.default)(ast, vistor);
    }
    ...
    ast.program.body.unshift(taroComponentsImport, reactImport, taroImport, withWeappImport, ...wxses.filter(wxs => !wxs.src.startsWith('./wxs__')).map(wxs => (0, utils_1.buildImportStatement)(wxs.src, [], wxs.module)));
    return ast;
}
exports.parseScript = parseScript;
function parsePage(pagePath, returned, componentType, refId, wxses, isApp = false) {
    const stateKeys = [];
    pagePath.traverse({
        CallExpression(path) {
            const callee = path.get('callee');
    ...
    if (arg.isObjectExpression() || arg.isIdentifier()) {
        ...
        throw (0, utils_1.codeFrameError)(arg.node, `${componentType || '组件'} 的第一个参数必须是一个对象或变量才能转换。`);
    }
    const wxsNames = new Set(wxses ? wxses.map(w => w.module) : []);
    const renderFunc = (0, utils_1.buildRender)(componentType === 'App'
